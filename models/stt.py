...
class SpatialTemporalTransformer(nn.Module):
    ...
    def _initialize_kv_cache(self, batch_size, dtype, device, local_attn_size=-1):
        """
        Initialize KV cache for all CausalTimeSpaceBlocks.
        
        Args:
            batch_size: Batch size
            dtype: Data type for cache tensors
            device: Device to create tensors on
            local_attn_size: Size of local attention window (-1 for global)
        
        Returns:
            List of KV cache dictionaries for temporal attention blocks
        """
        from models.modules.kv_cache_attention import initialize_kv_cache
        
        # Calculate cache size based on attention window
        if local_attn_size == -1:
            cache_size = 32760  # Global attention
        else:
            cache_size = local_attn_size * self.total_token_size
        
        # Initialize cache for each temporal attention block
        kv_cache_list = initialize_kv_cache(
            num_blocks=self.causal_time_space_num,
            batch_size=batch_size,
            cache_size=cache_size,
            n_heads=self.config.n_head,
            head_dim=self.C // self.config.n_head,
            dtype=dtype,
            device=device
        )
    ...
    @torch.no_grad()
    def evaluate_with_cache(self, feature, pose_total, yaw_total, 
                           kv_cache=None, current_frame=0, 
                           local_attn_size=-1, sample_last=True):
        """
        Evaluate with KV caching for efficient autoregressive generation.
        
        This method is similar to evaluate() but maintains KV cache across
        multiple calls for faster long-video generation.
        
        Args:
            feature: Input features [B, F, L, C]
            pose_total: Pose indices [B, (F+1)*block, 2]
            yaw_total: Yaw indices [B, (F+1)*block, 1]
            kv_cache: Optional KV cache from previous calls
            current_frame: Current frame index in the full sequence
            local_attn_size: Local attention window size (-1 for global)
            sample_last: Whether to return only the last frame's features
        
        Returns:
            auto_regressive_token_embeddings: Features for next stage
            pose_emb: Pose embeddings
            kv_cache: Updated KV cache (if initialized)
        """
        # Get embeddings for pose and yaw
        yaw_emb_total, pose_x_emb_total, pose_y_emb_total = self.get_yaw_pose_emb(pose_total, yaw_total)
        ...
        # Input embeddings (exclude last frame for prediction)
        input_pose_x_token_embeddings = pose_x_token_embeddings[:, :-1, ...]
        input_pose_y_token_embeddings = pose_y_token_embeddings[:, :-1, ...]
        input_yaw_token_embeddings = yaw_token_embeddings[:, :-1, ...]
        ...
        if kv_cache is None:
            kv_cache = self._initialize_kv_cache(
                batch_size=B,
                dtype=time_space_token_embeddings.dtype,
                device=time_space_token_embeddings.device,
                local_attn_size=local_attn_size
            )
        
        # Causal time and full space with KV caching
        # Note: For now, we're using the standard forward pass
        # In a future optimization, we can modify CausalTimeSpaceBlock 
        # to accept kv_cache parameter
        for i in range(self.causal_time_space_num):
        ...
        return auto_regressive_token_embeddings, pose_emb, kv_cache
